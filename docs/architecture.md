...
4. **File persistence** — stores encode validated snapshots below the configured
   state directory. Blob payloads live below the owned `blobs/objects` path;
   unrelated files are not part of reset or cleanup.
...
then reads or mutates the file-backed stores.
...
exactly-once delivery. Packaging and smoke scripts verify local artifacts only;
they do not deploy, publish, change repository visibility, or enable the GitHub
Wiki.
